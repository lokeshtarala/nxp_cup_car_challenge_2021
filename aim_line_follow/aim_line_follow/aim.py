...
class LineFollow(Node):
    ...
    def __init__(self):
        ...
        self.passedImage = None
        self.passedImage = cv2.imread("/home/basilv/ros2ws/src/aim_line_follow/aim_line_follow/passed.jpeg")
        ...
        #setup drive
        gauth = GoogleAuth()
        gauth.DEFAULT_SETTINGS['client_config_file'] = "/home/basilv/ros2ws/src/aim_line_follow/aim_line_follow/client_secrets.json"
    ...
    def pixyImageCallback(self,msg):
        ...
        cv2.imwrite("succssfull.jpeg",self.passedImage)
    ...
    def listener_callback(self, msg):
        ...
        if(num_vectors == 2):
            if(self.new_episode):
                cv2.imwrite(f"/home/basilv/ros2ws/src/aim_line_follow/aim_line_follow/{self.new_episode}_{self.iteration}_{num_vectors}.jpeg",self.passedImage)
                f = self.drive.CreateFile({'title': f"{self.new_episode}_{self.iteration}_{num_vectors}.jpeg",'parents':[{'id':'1U1tIilfX2rzKiQidcmEQEgaATi-Y2trt'}]})
                f.SetContentFile(f"/home/basilv/ros2ws/src/aim_line_follow/aim_line_follow/{self.new_episode}_{self.iteration}_{num_vectors}.jpeg")
                f.Upload()
                ...
                while(True):
                    file_list = self.drive.ListFile({'q': "'1U1tIilfX2rzKiQidcmEQEgaATi-Y2trt' in parents and trashed=false", }).GetList()
                    for file1 in file_list:
                        if(file1['title'] == f"{self.new_episode}_{self.iteration}_{num_vectors}.txt"):
                            ...
                            a = file_r.readline()
                            self.speed,self.steer,self.muSpeed,self.sigmaSpeed,self.muSteer,self.sigmaSteer = [float(x) for x in a.split(" ")]
                            breakFlag = True
                            break
                    if(breakFlag):
                        breakFlag = False
                ...
                file1 = self.drive.CreateFile({'title': f"{self.new_episode}_{self.iteration}_{num_vectors}R.txt" , 'parents':[{'id':'1U1tIilfX2rzKiQidcmEQEgaATi-Y2trt'}]})
